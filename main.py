...
ADMIN_IDS_RAW = os.getenv("7105046320") or ""
ADMIN_IDS = {
...
def subscription_kb() -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    kb.button(text="📲 Подключить устройство", callback_data="sub:howto")
...
def renew_kb() -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    kb.button(text="💸 Оплатить СБП", callback_data="pay:sbp:subscription")
    kb.button(text="🪙 Оплатить криптой", callback_data="pay:crypto:subscription")
...
async def cb_subscription(call: CallbackQuery):
    ...
    active = is_subscription_active(user)
    until = user["subscription_until"] if user and user["subscription_until"] else "Не активна"
    text = (
        "<b>🔐 Моя подписка</b>\n\n"
        f"🔗 Ссылка подписки:\n<code>{SUBSCRIPTION_URL}</code>\n\n"
        f"Статус: {'🟢 Активна' if active else '🔴 Истекла'}\n"
        f"Дата окончания: <b>{until}</b>\n"
        f"Тариф: <b>VPN доступ</b>\n"
        f"Цена: <b>{PRICE_MONTH_RUB} ₽ / месяц</b>\n"
        "Доступ: <b>безлимитный</b>"
    )
    await call.message.edit_text(text, reply_markup=subscription_kb())
    await call.answer()

@dp.callback_query(F.data == "sub:howto")
